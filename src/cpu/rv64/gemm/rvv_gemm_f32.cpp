...
void copy_A_optimized(
    ...
    for (dim_t k = 0; k < K; k++) {
        ...
        if (isTransA) {
            ...
            while (i < m) {
                size_t vl = (m - i <= vl_max) ? __riscv_vsetvl_e32m2(m - i) : vl_max;
                const float *a_ptr = A + i * lda + k;
                vfloat32m2_t v_a = __riscv_vlse32_v_f32m2(a_ptr, stride, vl);
                __riscv_vse32_v_f32m2(ws + i, v_a, vl);
                i += vl;
            }
        } else {
            ...
            while (i < m) {
                size_t vl = (m - i <= vl_max) ? __riscv_vsetvl_e32m2(m - i) : vl_max;
                vfloat32m2_t v_a = __riscv_vle32_v_f32m2(a_ptr + i, vl);
                __riscv_vse32_v_f32m2(ws + i, v_a, vl);
                i += vl;
            }
        }
...
void kernel_mxn_optimized(dim_t K, const float *A, const dim_t lda, const float *B,
    ...
    // 优化1: 使用更高的LMUL值以提高向量寄存器利用率
    // 根据VLEN动态选择LMUL，这里使用m2作为平衡点
    const size_t vl_max = __riscv_vsetvlmax_e32m2();
    const size_t vl_m = (m <= vl_max) ? __riscv_vsetvl_e32m2(m) : __riscv_vsetvlmax_e32m2();

    // 优化2: 使用固定大小数组避免VLA问题，n=4是固定的
    vfloat32m2_t v_c_acc_0, v_c_acc_1, v_c_acc_2, v_c_acc_3;
    v_c_acc_0 = __riscv_vfmv_v_f_f32m2(0.0f, vl_m);
    if (n > 1) v_c_acc_1 = __riscv_vfmv_v_f_f32m2(0.0f, vl_m);
    if (n > 2) v_c_acc_2 = __riscv_vfmv_v_f_f32m2(0.0f, vl_m);
    if (n > 3) v_c_acc_3 = __riscv_vfmv_v_f_f32m2(0.0f, vl_m);

    // 优化3: vsetvl提升到外层循环，减少调用频率
    for (dim_t k = 0; k < K; k++) {
        dim_t i = 0;
        while (i < m) {
            size_t vl = (m - i <= vl_max) ? __riscv_vsetvl_e32m2(m - i) : vl_max;
            ...
            // 优化4: 展开循环避免数组访问
            if (n >= 1) {
                float b0 = isTransB ? B[0 + k * ldb] : B[k + 0 * ldb];
                v_c_acc_0 = __riscv_vfmacc_vf_f32m2(v_c_acc_0, b0, v_a, vl);
            }
            if (n >= 2) {
                float b1 = isTransB ? B[1 + k * ldb] : B[k + 1 * ldb];
                v_c_acc_1 = __riscv_vfmacc_vf_f32m2(v_c_acc_1, b1, v_a, vl);
            }
            if (n >= 3) {
                float b2 = isTransB ? B[2 + k * ldb] : B[k + 2 * ldb];
                v_c_acc_2 = __riscv_vfmacc_vf_f32m2(v_c_acc_2, b2, v_a, vl);
            }
            if (n >= 4) {
                float b3 = isTransB ? B[3 + k * ldb] : B[k + 3 * ldb];
                v_c_acc_3 = __riscv_vfmacc_vf_f32m2(v_c_acc_3, b3, v_a, vl);
            }
            i += vl;
        }
    }
    ...
    if (n >= 1) {
        ...
        while (i < m) {
            size_t vl = (m - i <= vl_max) ? __riscv_vsetvl_e32m2(m - i) : vl_max;
            float *c_final_ptr = C + 0 * ldc + i;
            vfloat32m2_t v_res;
            if (beta == 0.0f) {
                v_res = __riscv_vfmul_vf_f32m2(v_c_acc_0, alpha, vl);
            } else {
                vfloat32m2_t v_c_old = __riscv_vle32_v_f32m2(c_final_ptr, vl);
                v_res = __riscv_vfmul_vf_f32m2(v_c_old, beta, vl);
                v_res = __riscv_vfmacc_vf_f32m2(v_res, alpha, v_c_acc_0, vl);
            }
    ...
    if (n >= 2) {
        ...
        while (i < m) {
            size_t vl = (m - i <= vl_max) ? __riscv_vsetvl_e32m2(m - i) : vl_max;
            float *c_final_ptr = C + 1 * ldc + i;
            vfloat32m2_t v_res;
            if (beta == 0.0f) {
                v_res = __riscv_vfmul_vf_f32m2(v_c_acc_1, alpha, vl);
            } else {
                vfloat32m2_t v_c_old = __riscv_vle32_v_f32m2(c_final_ptr, vl);
                v_res = __riscv_vfmul_vf_f32m2(v_c_old, beta, vl);
                v_res = __riscv_vfmacc_vf_f32m2(v_res, alpha, v_c_acc_1, vl);
            }
            __riscv_vse32_v_f32m2(c_final_ptr, v_res, vl);
            i += vl;
        }
    ...
    if (n >= 3) {
        ...
        while (i < m) {
            size_t vl = (m - i <= vl_max) ? __riscv_vsetvl_e32m2(m - i) : vl_max;
            float *c_final_ptr = C + 2 * ldc + i;
            vfloat32m2_t v_res;
            if (beta == 0.0f) {
                v_res = __riscv_vfmul_vf_f32m2(v_c_acc_2, alpha, vl);
            } else {
                vfloat32m2_t v_c_old = __riscv_vle32_v_f32m2(c_final_ptr, vl);
                v_res = __riscv_vfmul_vf_f32m2(v_c_old, beta, vl);
                v_res = __riscv_vfmacc_vf_f32m2(v_res, alpha, v_c_acc_2, vl);
            }
            __riscv_vse32_v_f32m2(c_final_ptr, v_res, vl);
            i += vl;
        }
    ...
    if (n >= 4) {
        ...
        while (i < m) {
            size_t vl = (m - i <= vl_max) ? __riscv_vsetvl_e32m2(m - i) : vl_max;
            float *c_final_ptr = C + 3 * ldc + i;
            vfloat32m2_t v_res;
            if (beta == 0.0f) {
                v_res = __riscv_vfmul_vf_f32m2(v_c_acc_3, alpha, vl);
            } else {
                vfloat32m2_t v_c_old = __riscv_vle32_v_f32m2(c_final_ptr, vl);
                v_res = __riscv_vfmul_vf_f32m2(v_c_old, beta, vl);
                v_res = __riscv_vfmacc_vf_f32m2(v_res, alpha, v_c_acc_3, vl);
            }
            __riscv_vse32_v_f32m2(c_final_ptr, v_res, vl);
            i += vl;
        }
...
void block_ker(const dim_t M, const dim_t N, const dim_t K, const float *A,
    ...
    // 优化选择：根据问题规模和VLEN选择最优kernel
    const size_t vlen = __riscv_vsetvlmax_e32m1() * 32; // 估算VLEN
    const bool use_optimized = (vlen >= 128) && (K >= 32) && (M >= 16) && (N >= 4);
    printf("🎯 优化选择: vlen=%zu, K=%ld, M=%ld, N=%ld, use_optimized=%s\n",
           vlen, K, M, N, use_optimized ? "YES" : "NO");
    ...
    for (dim_t i = 0; i < Mu; i += unroll_factor<float>::m) {
        for (dim_t j = 0; j < Nu; j += unroll_factor<float>::n) {
            ...
            if (do_copy) {
                if (j == 0) {
                    if (use_optimized) {
                        copy_A_optimized(isTransA, K, a, lda, ws);
                    } else {
                        copy_A(isTransA, K, a, lda, ws);
                    }
                }
                if (use_optimized) {
                    kernel_mxn_optimized<false, isTransB>(K, ws, unroll_factor<float>::m, b,
                ...
                if (use_optimized) {
                    kernel_mxn_optimized<isTransA, isTransB>(K, a, lda, b, ldb,
...
dnnl_status_t rvv_gemm_f32(const char *transa_, const char *transb_,
    ...
    // 添加调试输出确认我们的函数被调用
    printf(" RVV GEMM F32 CALLED: M=%ld, N=%ld, K=%ld\n", *M_, *N_, *K_);

    // 检查VLEN配置
    const size_t vl_debug = __riscv_vsetvlmax_e32m1();
    const size_t vlen_debug = vl_debug * 32;
    printf("VLEN检查: vl_max=%zu, estimated_vlen=%zu\n", vl_debug, vlen_debug);
